...
class AntimonyWrapper {
    ...
    constructor(AntimonyModule) {
        ...
        this.loadString = AntimonyModule.cwrap('loadString', 'number', [
            'number',
        ])
        this.loadAntimonyString = AntimonyModule.cwrap(
            'loadAntimonyString',
            'number',
            ['number']
        )
        this.loadSBMLString = AntimonyModule.cwrap('loadSBMLString', 'number', [
            'number',
        ])
        this.getSBMLString = AntimonyModule.cwrap('getSBMLString', 'string', [
            'null',
        ])
        this.getAntimonyString = AntimonyModule.cwrap(
            'getAntimonyString',
            'string',
            ['null']
        )
        this.getCompSBMLString = AntimonyModule.cwrap(
            'getCompSBMLString',
            'string',
            ['string']
        )
        this.clearPreviousLoads = AntimonyModule.cwrap(
            'clearPreviousLoads',
            'null',
            ['null']
        )
        this.getLastError = AntimonyModule.cwrap('getLastError', 'string', [
            'null',
        ])
        this.getWarnings = AntimonyModule.cwrap('getWarnings', 'string', [
            'null',
        ])
        this.getSBMLInfoMessages = AntimonyModule.cwrap(
            'getSBMLInfoMessages',
            'string',
            ['string']
        )
        this.getSBMLWarnings = AntimonyModule.cwrap(
            'getSBMLWarnings',
            'string',
            ['string']
        )
